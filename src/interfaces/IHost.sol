...
interface IHost {
    ...
    /// @notice Check if the asset is whitelisted in Host for revenue registration.
    ...
    /// @notice Register revenue generated by the unit in Host.
    /// @param symbol DAO symbol
    /// @param unitId Unit identifier
    /// @param asset Address of the asset in which revenue is generated. The asset must be whitelisted in Host
    /// @param amount Amount of revenue generated by the unit. This amount will be taken from the sender.
    function revenue(string calldata symbol, string memory unitId, address asset, uint amount) external;
    ...
    /// @notice Whitelist assets for revenue processing
    /// @custom:restricted Restricted through access manager (only admin)
    ...
    function whitelistAssets(address[] memory assets_, bool whitelisted) external;
