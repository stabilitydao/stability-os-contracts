...
contract DeployAuthorityAndHostAndMevbots is Script {
    ...
    function run() external {
        ...
        /// @dev setup Host Chain Settings
        IHost.HostChainSettings memory chainSettings = IHost.HostChainSettings({
            exchangeAsset: USDC,
            hostBridge: address(0),
            timelock: 86400,
            dataReader: address(hostBridgeReader[2])
        });
        IHost(TARGET_HOST_MAINNET).setChainSettings(chainSettings);

        IHost.HostSettings memory settings = HostSetupLib.getDefaultHostSettings();
        IHost(TARGET_HOST_MAINNET).setSettings(settings);

        /// @dev SEED token
        address seedImplementation = address(new SeedToken());
        IHost(TARGET_HOST_MAINNET).setContractImplementation(uint(IDAOData.ContractIndices.SEED_TOKEN_1), seedImplementation);

        /// grant roles to Host
        authority.grantRole(AccessRolesLib.HOST_TOKEN_MINTER, address(hostBridgeReader[0]), 0);
        authority.grantRole(AccessRolesLib.CONTRACTS_UPGRADER, address(hostBridgeReader[0]), 0);
